...
def get_escalate_alert_policies(data):
    try:
        #url = "https://{0}/api/v2/tenants/{1}/scheduleMaintenances".format(API_SERVER, CLIENT_ID)
        url = "https://api.vistara.io/api/v2/tenants/msp_588058/escalations/search"
        headers = {'Content-Type': 'application/json', "Authorization" : "Bearer " + "13181428-3e0e-429b-bafa-87d3ba997c66"}
        #result = httpRequest(url, policy_payload(), headers, 'POST')
        #print(result)
        data = NULL_payload()
        print data
        result = json.loads(httpRequest(url, None, headers, 'GET'))
        ###print result['description']
        mydict = result.get("results", {})
        #print mydict
        for vals in mydict:
        ...
        print ("Failed to get  escalate  policies: " +str(e))
...
get_escalate_alert_policies(None)
